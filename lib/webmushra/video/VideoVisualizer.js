...
function VideoVisualizer(_parent, _conditions) {
  ...
  this.conditions.forEach(function (condition, i) {
    ...
    videoElement = $(`
    <video 
      id="vimeo_video_player_`+i+`" class="video-element" 
      class="video-element" 
      src = "`+ condition.getFilepath() +`"      
      height=400 >
    `).get(0);
...
VideoVisualizer.prototype.playCondition = function (_index) {
  ...
  this.videoElements.forEach(function (videoElement, i) {
    if (i == _index) {
      ...
      var getPaused = function (player) {
        return new Promise((resolve, reject) => resolve(player.paused));
      }
      getPaused(player).then(function(paused) {
        if (!paused) {
          player.pause();
